<template>
    <section class="row overlayable">
        <data-process :process="dataProcess" :with_loader="true"></data-process>

        <div class="col-xs-12 col-sm-12 col-md-12" v-if="!this.qrcode && dataIsReady">
                <button class="btn btn-md btn-success"
                        title="Generate Build Status QR Code"
                        v-on:click="openQrGenerateModal">
                    <i class="fa fa-qrcode" aria-hidden="true"></i> Generate Build Status QR Code
                </button>
        </div>

        <div class="row" v-if="this.qrcode && dataIsReady">
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                        <section class="panel sub-panel">
                            <header class="panel-heading">
                                <h4 class="panel-title">Building Build QR Code Details</h4>
                            </header>
                            ...
                            <div class="panel-footer">
                                <div class="btn-group">
                                     <button type="button" class="btn btn-md btn-success"
                                            title="Regenerate Build Status QR Code"
                                            v-on:click="openQrGenerateModal">
                                        <i class="fa fa-qrcode" aria-hidden="true"></i> Regenerate Build Status QR Code
                                    </button>
        ...
        <component v-if="qrGenerateModal"
                   ...
                   transition="modal"
                   :item="item"
                   :params=" { modaltitle: 'Generate Building Build Status Update QR Code' ,type: 'build' } "
